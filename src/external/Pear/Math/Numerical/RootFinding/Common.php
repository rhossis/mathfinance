 ...
 * @version    CVS: $Id: Common.php 272242 2008-12-29 16:32:47Z firman $
 */
 ...
 * @package    Math_Numerical_RootFinding
 * @subpackage Methods
 * @author     Original: Firman Wandayandi <firman@php.net>
 * @copyright  2004-2008 Firman Wandayandi
 * @license    http://www.opensource.org/licenses/bsd-license.php BSD License
...
class Math_Numerical_RootFinding_Common
{
    ...
    function set($option, $value = null)
    {
        if (!is_array($option) && !is_string($option)) {
            //return ('Type mismatch for $option argument');
            return false;
        }
    ...
    {
        $err = $this->getEqResult($eqFunction, $guess);
        if($err === false) {
            return $err;
        }
        return true;
